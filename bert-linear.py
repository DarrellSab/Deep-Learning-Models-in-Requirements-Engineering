...
RESULTS_CSV = "results_10runs_bert_lstm_linear.csv"
...
class QALinearModel(nn.Module):
    def __init__(self, input_size=768, dense_size=512):
        super(QALinearModel, self).__init__()
        self.projection = nn.Linear(input_size, dense_size)
...
# ============================
@torch.no_grad()
def get_lstm_answer(q: str, ctx: str, qa_model: QALinearModel) -> str:
    enc = tokenizer(
        q, ctx,
...
def evaluate_requirements_json(path: str, qa_model: QALinearModel) -> Tuple[float, float, float]:
...
def train_one_epoch(model: QALinearModel, loader: DataLoader, optimizer) -> Tuple[float, float]:
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
...
@torch.no_grad()
def validate(model: QALinearModel, loader: DataLoader) -> Tuple[float, float]:
    model.eval()
    total_loss = 0.0
    correct = 0
    total = 0
