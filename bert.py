...
RESULTS_CSV = "results_10runs_bert.csv"
...
# ============================
# 6) BERT QA model (span heads)
# ============================
class BertQAModel(nn.Module):
    def __init__(self, model_name="bert-base-cased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(model_name)
        self.start_head = nn.Linear(self.bert.config.hidden_size, 1)
        self.end_head = nn.Linear(self.bert.config.hidden_size, 1)
...
# ============================
@torch.no_grad()
def get_answer(model: BertQAModel, question: str, context: str) -> str:
    enc = tokenizer(
        question, context,
        truncation="only_second",
        padding="max_length",
...
def evaluate_re_json(model: BertQAModel, path: str) -> Tuple[float, float, float]:
...
def run_one_seed(features: List[Dict], seed: int) -> Dict:
    ...
    model = BertQAModel("bert-base-cased").to(device)
