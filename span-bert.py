...
RESULTS_CSV = "results_10runs_spanbert.csv"
...
# 3) Tokenizer
# ============================
tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, trust_remote_code=False,local_files_only=False) # first running local should be False
...
def train_one_epoch(model, loader, optimizer) -> Tuple[float, float]:
    ...
    for batch in loader:
        ...
        with amp_ctx:
            ...
            s_log, e_log = outputs.start_logits, outputs.end_logits
...
# ============================
# 8) Span decoding for RE evaluation (context-only)
# ============================
@torch.no_grad()
def get_answer(model: nn.Module, question: str, context: str) -> str:
    enc = tokenizer(
        question, context,
        truncation="only_second",
        padding="max_length",
...
def evaluate_re_json(model: nn.Module, path: str) -> Tuple[float, float, float]:
...
# ============================
# 11) Run one seed with Early Stopping
# ============================
